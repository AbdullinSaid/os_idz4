    ...
    for (int i = 1; i <= 10; ++i) {
        if (fork() == 0) {
            for (;;)
            {
                ...
                if ((recvMsgSize = recvfrom(servSock1, echoBuffer, 100, 0,
                    (struct sockaddr *) &echoClntAddr, &cliAddrLen)) < 0)
                    DieWithError("recvfrom() failed");
                int requiredBook = atoi(echoBuffer);
                char sendString[100];
                int sendStringLen;
                sendStringLen = snprintf(sendString, "Client %s required book number %d\n", inet_ntoa(echoClntAddr.sin_addr), requiredBook);
                printf("Client %s required book number %d\n", inet_ntoa(echoClntAddr.sin_addr), requiredBook);
                ...
                sendStringLen = snprintf(sendString, "Client %s took book number %d\n", inet_ntoa(echoClntAddr.sin_addr), requiredBook);
                printf("Client %s took book number %d\n", inet_ntoa(echoClntAddr.sin_addr), requiredBook);
                if (sendto(servSock2, sendString, sendStringLen, 0, (struct sockaddr *)
                    &multicastAddr, sizeof(multicastAddr)) != sendStringLen)
                    DieWithError("sendto() sent a different number of bytes than expected");
                sleep(10);
                ...
                printf("Client %s walked home\n", inet_ntoa(echoClntAddr.sin_addr));
                sendStringLen = snprintf(sendString, "Client %s walked home\n", inet_ntoa(echoClntAddr.sin_addr));
                if (sendto(servSock2, sendString, sendStringLen, 0, (struct sockaddr *)
                    &multicastAddr, sizeof(multicastAddr)) != sendStringLen)
                    DieWithError("sendto() sent a different number of bytes than expected");
            }
